 ...
 *		ExecScanInc
 *
 *		    totem: Incremental version of ExecScan
 *
 * ----------------------------------------------------------------
 */
TupleTableSlot *
ExecScanInc(ScanState *node,
		 ExecScanAccessMtd accessMtd,	/* function returning a tuple */
		 ExecScanRecheckMtd recheckMtd)
{
	ExprContext *econtext;
	...
	for (;;)
	{
        ...
        else if (incInfo->leftAction == PULL_DELTA) 
        {
            ...
            elog(ERROR, "%d: PullAction %d not supported", incInfo->id, incInfo->leftAction); 
 ...
 *		ExecScanDBT
 *
 *		    totem: DBT version of Scan
 *
 * ----------------------------------------------------------------
 */
TupleTableSlot *
ExecScanDBT(ScanState *node,
		 ExecScanAccessMtd accessMtd,	/* function returning a tuple */
		 ExecScanRecheckMtd recheckMtd)
{
	ExprContext *econtext;
 ...
 *	    InitScanDBT
 *
 *		    totem: DBT version for Scanning
 *
 * ----------------------------------------------------------------
 */
void
ExecInitScanDBT(ScanState *node) 
{ 
    node->tq_reader = NULL; 
} 

void 
ExecTakeDeltaDBT(ScanState *node)
{
    IncTQPool *tq_pool = node->ps.state->tq_pool; 
    node->tq_reader = GetTQReader(tq_pool, node->ss_currentRelation, node->tq_reader);
}

TupleTableSlot *
ExecProjectDBT(ScanState *node)
{	
    ExprContext *econtext;
	ProjectionInfo *projInfo;
    ...
    econtext->ecxt_scantuple = node->ps.ps_WorkingTupleSlot; 
    return ExecProject(projInfo);
}
