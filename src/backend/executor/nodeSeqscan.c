...
/*
 * totem: add incremental scanning
 */
 ...
 */
static TupleTableSlot *
ExecSeqScanInc(PlanState *pstate)
{
	SeqScanState *node = castNode(SeqScanState, pstate);
 ...
 */
void
ExecResetSeqScanState(SeqScanState * node)
{
    return; 
}

/*-----------------------------------------------------------------
 *      ExecInitSeqScanDelta
 *
 *      totem: Init states for processing delta 
 *
 * ----------------------------------------------------------------
 */
void
ExecInitSeqScanDelta(SeqScanState * node)
{
    ...
    node->ss.ps.chgAction = incInfo->leftAction; 
