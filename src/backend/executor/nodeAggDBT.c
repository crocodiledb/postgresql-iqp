...
static void agg_finialize_hash_table(AggState *aggstate); 
static void agg_insert_one_tuple(AggState *aggstate); 
...
static void
agg_insert_one_tuple(AggState *aggstate)
{
	TupleTableSlot *outerslot = aggstate->ss.ps.ps_WorkingTupleSlot; 
...
static void
agg_finialize_hash_table(AggState *aggstate)
{
    aggstate->table_filled = true;
 ...
 */
static TupleTableSlot *
ExecAggDBT(PlanState *pstate)
{
    AggState *node = castNode(AggState, pstate); 

	if (!node->table_filled)
    {
        agg_insert_one_tuple(node); 
        return NULL; 
    }
 ...
 */
void 
ExecInitAggDBT(AggState *aggstate)
{
    aggstate->ss.ps.ExecProcNode = ExecAggDBT; 
}

void 
ExecFinalizeAggDBT(AggState *aggstate)
{
    agg_finialize_hash_table(aggstate); 
}


void 
ExecResetAggDBT(AggState *aggstate)
{
