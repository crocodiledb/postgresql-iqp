...
/*
 * tuplesort_reverttoheap
 *      totem: revert to heap status and allow new tuples inserted
 */
void
tuplesort_reverttoheap(Tuplesortstate *state) 
{
	MemoryContext oldcontext = MemoryContextSwitchTo(state->sortcontext);
	...
	switch (state->status)
	{
		case TSS_SORTEDINMEM:
			...
			break;
		default:
			elog(ERROR, "totem: invalid tuplesort state");
			break;
	}
